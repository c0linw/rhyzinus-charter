...
func export_data(chart_node: Dictionary) -> Dictionary:
	# convert notes and timing points to relevant format, then do the same thing the game's loader does
	timing_points = chart_node.bpm_changes.duplicate()
	timing_points.append_array(chart_node.velocity_changes.duplicate())
	notes = chart_node.notes.duplicate()
	
	# original game loader code starts here
